...
router.get("/:name", (req, res) => {
  ...
  import_profile_svc.default.get(name).then((profile) => res.json(profile)).catch((err) => res.status(404).send(err));
});
router.post("/", (req, res) => {
...
router.put("/:name", (req, res) => {
  const { name } = req.params;
  const newProfile = req.body;
  import_profile_svc.default.update(name, newProfile).then((profile) => res.json(profile)).catch((err) => res.status(404).end());
});
router.delete("/:name", (req, res) => {
  const { name } = req.params;
  import_profile_svc.default.remove(name).then(() => res.status(204).end()).catch((err) => res.status(404).send(err));
});
